...
Benchmarks all 5 YOLO11 ncnn models on a full sticky-card image
using manual SAHI tiling + NMS. Outputs results/ncnn_results.csv.
...
# ─── Config ───────────────────────────────────────────────────────────────────
NCNN_MODELS_DIR = Path("/home/paalab/Documents/yolo/ncnn_models")
IMG_PATH        = Path("/home/paalab/Documents/yolo/images/0727_0836_640.jpg")
...
MODELS = {
    "yolo11n_b8":  NCNN_MODELS_DIR / "yolo11n_b8"  / "best_ncnn_model",
    "yolo11s_b4":  NCNN_MODELS_DIR / "yolo11s_b4"  / "best_ncnn_model",
    "yolo11m_b16": NCNN_MODELS_DIR / "yolo11m_b16" / "best_ncnn_model",
    "yolo11l_b4":  NCNN_MODELS_DIR / "yolo11l_b4"  / "best_ncnn_model",
    "yolo11x_b8":  NCNN_MODELS_DIR / "yolo11x_b8"  / "best_ncnn_model",
}
...
def run_inference(model, img) -> int:
    ...
    for y in range(0, H, stride):
        for x in range(0, W, stride):
            ...
            results = model(tile, imgsz=TILE_SIZE, conf=CONF_THRESHOLD, verbose=False)
            r = results[0]
...
def benchmark_model(name: str, model_dir: Path, img) -> dict:
    print(f"\n{'='*50}")
...
def save_csv(rows: list[dict]) -> None:
    ...
    path = OUTPUT_DIR / "ncnn_results.csv"
...
def main():
    print("YOLO11 ncnn Benchmark Raspberry Pi 4B")
    ...
    for name, model_dir in MODELS.items():
        row = benchmark_model(name, model_dir, img)
        if row:
            rows.append(row)
